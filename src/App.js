...
// ─── CONFIG SUPABASE (reemplaza con tus credenciales al instalar) ──────────
...
const USE_SUPABASE = false; // ← cambiar a true cuando instales localmente
...
const db = {
  getUsers:     ()         => sbFetch("users?select=*&order=id.asc"),
  getRecords:   ()         => sbFetch("records?select=*&order=entry.desc"),
  ...
  updateRecord: (id, body) => sbFetch(`records?id=eq.${id}`, { method:"PATCH", body:JSON.stringify(body) }),
  ...
  updateUser:   (id, body) => sbFetch(`users?id=eq.${id}`, { method:"PATCH", body:JSON.stringify(body) }),
};

// ─── DATOS LOCALES (prototipo) ─────────────────────────────────────────────
...
const LOCAL_USERS = [
  { id:1, name:"Carlos Méndez",  email:"carlos@empresa.com",  documento:"11111111", password:"1234",     role:"employee", avatar:"CM", schedule:{...DEFAULT_SCHEDULE} },
  { id:2, name:"Laura Ríos",     email:"laura@empresa.com",   documento:"22222222", password:"1234",     role:"employee", avatar:"LR", schedule:{...DEFAULT_SCHEDULE} },
  { id:3, name:"Miguel Torres",  email:"miguel@empresa.com",  documento:"33333333", password:"1234",     role:"employee", avatar:"MT", schedule:{...DEFAULT_SCHEDULE} },
  { id:4, name:"Admin Sistema",  email:"admin@empresa.com",   documento:"44444444", password:"admin123", role:"admin",    avatar:"AS", schedule:{...DEFAULT_SCHEDULE} },
];
...
function exportCSV(records, users) {
  const rows = [["Empleado","Correo","Fecha","Entrada","Salida","Horas","Puntualidad"]];
  records.forEach(r => {
    const u = users.find(x=>x.id===r.user_id||x.id===r.userId);
    const lm = isLate(r,u);
    const punt = lm===null?"—":lm>0?`Tarde ${lm}min`:`Temprano ${Math.abs(lm)}min`;
    rows.push([u?.name||"—",u?.email||"—",fmtDate(r.entry),fmtShort(r.entry),r.exit?fmtShort(r.exit):"—",r.exit?diffH(r.entry,r.exit)+"h":"—",punt]);
  });
  ...
  return new Promise(resolve => {
    ...
    reader.onload = e => {
      try {
        ...
        if(emailIdx===-1){resolve({ok:false,error:"No se encontró la columna 'correo'."});return;}
        const updated=[],errors=[];
        rows.slice(1).forEach((row,i)=>{
          const email=String(row[emailIdx]||"").trim().toLowerCase();
          const user=users.find(u=>u.email.toLowerCase()===email);
          if(!user){errors.push(`Fila ${i+2}: correo '${email}' no encontrado`);return;}
          const sched={};
...
function Badge({children,color="green"}){
  const c={green:"bg-emerald-500/20 text-emerald-300 border-emerald-500/30",red:"bg-red-500/20 text-red-300 border-red-500/30",amber:"bg-amber-500/20 text-amber-300 border-amber-500/30",blue:"bg-blue-500/20 text-blue-300 border-blue-500/30",purple:"bg-purple-500/20 text-purple-300 border-purple-500/30",slate:"bg-slate-500/20 text-slate-300 border-slate-500/30",cyan:"bg-cyan-500/20 text-cyan-300 border-cyan-500/30"};
  return <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${c[color]}`}>{children}</span>;
}
    ...
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="card p-6 w-full max-w-md" style={{fontFamily:"'DM Sans'",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div className="space-y-2.5">
          {Object.entries(sched).map(([day,s])=>(
            <div key={day} className={`rounded-xl p-3 border transition-all ${s.active?"bg-slate-800/60 border-slate-600/50":"bg-slate-800/20 border-slate-700/30 opacity-60"}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button onClick={()=>toggle(day)} className={`w-9 h-5 rounded-full transition-all relative flex-shrink-0 ${s.active?"bg-cyan-500":"bg-slate-600"}`}><span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${s.active?"left-4":"left-0.5"}`}/></button>
                  <span className="text-sm font-medium text-slate-200 w-12">{DAYS_LABEL[day]}</span>
                </div>
...
function AddUserModal({onSave,onClose}){
  const [form,setForm]=useState({name:"",email:"",documento:"",password:"",role:"employee"});
  const [err,setErr]=useState(""); const [saving,setSaving]=useState(false);
  const set=(k,v)=>setForm(p=>({...p,[k]:v}));
  const save=async()=>{
    if(!form.name.trim()||!form.documento.trim()||!form.password.trim()){setErr("Completa todos los campos obligatorios");return;}
    setSaving(true); setErr("");
    try{await onSave({...form,avatar:getInits(form.name),schedule:JSON.parse(JSON.stringify(DEFAULT_SCHEDULE))});}
    ...
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="card p-6 w-full max-w-sm" style={{fontFamily:"'DM Sans'"}}>
        ...
        <div className="space-y-3">
          ...
          <Inp label="Correo electrónico (opcional)" type="email" value={form.email} onChange={e=>set("email",e.target.value)} placeholder="maria@empresa.com"/>
          <Inp label="Contraseña inicial" type="text" value={form.password} onChange={e=>set("password",e.target.value)} placeholder="Contraseña inicial"/>
          <div><label className="block text-xs text-slate-400 mb-1">Rol</label><select value={form.role} onChange={e=>set("role",e.target.value)} className="w-full bg-slate-800/60 border border-slate-600/60 text-white rounded-xl px-3 py-2.5 text-sm"><option value="employee">Empleado</option><option value="admin">Administrador</option></select></div>
  ...
  return(
    <div className="modal-bg">
      <div className="card p-6 w-full max-w-sm text-center" style={{fontFamily:"'DM Sans'"}}>
        ...
        <h3 className="font-bold text-white mb-2" style={{fontFamily:"'Space Grotesk'"}}>Eliminar usuario</h3>
        <p className="text-slate-400 text-sm mb-5">¿Seguro que quieres eliminar a <span className="text-white font-semibold">{user.name}</span>?</p>
        <div className="flex gap-3"><button onClick={onClose} className="btn-ghost flex-1 rounded-xl py-2.5 text-sm">Cancelar</button><button onClick={go} disabled={loading} className="btn-danger flex-1 text-white font-semibold rounded-xl py-2.5 text-sm flex items-center justify-center gap-2 disabled:opacity-60">{loading&&<Spin/>}Eliminar</button></div>
      </div>
    </div>
  );
}
      ...
      <div className="card p-6 w-full max-w-lg" style={{fontFamily:"'DM Sans'",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 mb-4">
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center flex-shrink-0">📥</div>
        ...
        <div className={`drop-zone p-6 text-center cursor-pointer mb-4 ${drag?"drag-over":""}`} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={onDrop} onClick={()=>ref.current.click()}>
          ...
          {!file?<><div className="text-3xl mb-2">📂</div><p className="text-sm text-slate-300 font-medium">Arrastra tu Excel aquí</p><p className="text-xs text-slate-500 mt-1">o haz clic para seleccionar</p></>:<><div className="text-3xl mb-2">📊</div><p className="text-sm text-slate-300 font-medium">{file.name}</p><p className="text-xs text-slate-500 mt-1">{(file.size/1024).toFixed(1)} KB</p></>}
        </div>
        ...
        {result&&!loading&&(!result.ok
          ?<div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-sm text-red-300 mb-4">❌ {result.error}</div>
          :<div className="space-y-3 mb-4">
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4"><p className="text-sm font-semibold text-emerald-300 mb-2">✅ {result.updated.length} empleado{result.updated.length!==1?"s":""} encontrado{result.updated.length!==1?"s":""}</p><div className="space-y-1 max-h-36 overflow-y-auto scrollbar">{result.updated.map(u=><div key={u.id} className="flex items-center gap-2 text-xs text-slate-300"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0"/><span className="font-medium">{u.name}</span><span className="text-slate-500">horario actualizado</span></div>)}</div></div>
            {result.errors.length>0&&<div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-amber-300 mb-1">⚠️ {result.errors.length} advertencia(s)</p>{result.errors.map((e,i)=><p key={i} className="text-xs text-amber-200/70">{e}</p>)}</div>}
          </div>
        )}
...
function EmployeePanel({user,records,onEntry,onExit,onLogout,actionLoading}){
  ...
  const uid    = user.id;
  const mine   = records.filter(r=>(r.user_id||r.userId)===uid).sort((a,b)=>new Date(b.entry)-new Date(a.entry));
...
// ─── PANEL ADMIN ───────────────────────────────────────────────────────────
function AdminPanel({users,setUsers,records,setRecords,onLogout}){
  ...
  const saveSchedule=async(uid,sched)=>{
    if(USE_SUPABASE) await db.updateUser(uid,{schedule:sched});
    setUsers(p=>p.map(u=>u.id===uid?{...u,schedule:sched}:u)); setSchedUser(null);
  };
  const addUser=async data=>{
    let newUser={...data};
    if(USE_SUPABASE){const res=await db.insertUser(data);if(res&&res[0])newUser=res[0];}
    else newUser={...data,id:Date.now()};
    setUsers(p=>[...p,newUser]); setShowAdd(false);
  };
  const deleteUser=async id=>{
    if(USE_SUPABASE) await db.deleteUser(id);
    setUsers(p=>p.filter(u=>u.id!==id)); setDelUser(null);
  };
  const applyImport=async updated=>{
    if(USE_SUPABASE){for(const u of updated)await db.updateUser(u.id,{schedule:u.schedule});}
  ...
  return(
    <div className="min-h-screen bg-[#080d1a] text-white" style={{fontFamily:"'DM Sans'"}}>
      ...
      {delUser&&<ConfirmModal user={delUser} onConfirm={()=>deleteUser(delUser.id)} onClose={()=>setDelUser(null)}/>}
      {showAdd&&<AddUserModal onSave={addUser} onClose={()=>setShowAdd(false)}/>}
        ...
        {tab==="records"&&<>
          ...
          <div className="card overflow-hidden">
            ...
            {filtRecs.length===0?<p className="text-slate-500 text-sm text-center py-10">Sin registros</p>
              :<div className="overflow-x-auto scrollbar"><table className="w-full">
                <thead><tr className="border-b border-slate-700/30">{["Empleado","Fecha","Entrada","Salida","Horas","Puntualidad","Estado"].map(h=><th key={h} className="text-left text-xs text-slate-500 uppercase tracking-wider px-4 py-3 font-medium whitespace-nowrap">{h}</th>)}</tr></thead>
                <tbody className="divide-y divide-slate-700/20">{filtRecs.map(r=>{
                  const u=users.find(x=>x.id===(r.user_id||r.userId)); const lm=isLate(r,u);
                  return(<tr key={r.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-3"><div className="flex items-center gap-2"><Av name={u?.name||"?"} size="sm"/><div><p className="text-sm font-medium text-white">{u?.name}</p><p className="text-xs text-slate-500">{u?.email}</p></div></div></td>
        ...
        {tab==="users"&&<>
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              <button onClick={()=>setShowImport(true)} className="bg-slate-800/70 border border-slate-600/50 hover:border-emerald-500/50 hover:text-emerald-300 text-slate-300 text-sm font-medium rounded-xl px-4 py-2 flex items-center gap-2 transition-all">📊 Importar horarios Excel</button>
          ...
          <div className="card overflow-hidden">
            <div className="divide-y divide-slate-700/20">{users.map(u=>(
              <div key={u.id} className="px-5 py-4 flex items-center justify-between hover:bg-slate-800/20 transition-colors">
                <div className="flex items-center gap-3"><Av name={u.name}/><div><div className="flex items-center gap-2"><p className="text-sm font-medium">{u.name}</p><Badge color={u.role==="admin"?"purple":"blue"}>{u.role==="admin"?"Admin":"Empleado"}</Badge></div><p className="text-xs text-slate-500 mt-0.5">{u.email}</p></div></div>
                <div className="flex items-center gap-2">
                  {u.role==="employee"&&<button onClick={()=>setSchedUser(u)} className="text-xs btn-ghost rounded-lg px-3 py-1.5 flex items-center gap-1.5">🗓️ Horario</button>}
                  {u.role!=="admin"&&<button onClick={()=>setDelUser(u)} className="text-xs bg-slate-700/50 hover:bg-red-500/20 hover:text-red-300 border border-slate-600/40 hover:border-red-500/40 text-slate-400 rounded-lg px-3 py-1.5 transition-all">🗑️</button>}
                </div>
              </div>
            ))}</div>
          </div>
  ...
  const go=async()=>{
    ...
    try{
      ...
      if(USE_SUPABASE){
        ...
        user=res[0]; if(user.password!==pw){setErr("Documento o contraseña incorrectos");setLoad(false);return;}
      } else {
        user=users.find(u=>u.documento===doc.trim()&&u.password===pw);
        if(!user){setErr("Documento o contraseña incorrectos");setLoad(false);return;}
      }
      onLogin(user);
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="card p-7" style={{boxShadow:"0 0 40px rgba(6,182,212,.1)"}}>
          ...
          <div className="mt-5 pt-5 border-t border-slate-700/50">
            <p className="text-xs text-slate-500 text-center mb-3">Cuentas de prueba</p>
            <div className="grid grid-cols-2 gap-2">{[{l:"Empleado",e:"11111111",p:"1234"},{l:"Admin",e:"44444444",p:"admin123"}].map(d=><button key={d.l} onClick={()=>{setDoc(d.e);setPw(d.p);}} className="text-xs bg-slate-800/60 border border-slate-600/40 hover:border-cyan-500/40 text-slate-400 hover:text-cyan-300 rounded-lg px-3 py-2 transition-all text-left"><span className="font-semibold block text-slate-300">{d.l}</span>Doc: {d.e}</button>)}</div>
          </div>
        </div>
      </div>
    </div>
...
export default function App(){
  ...
  const loadRemote=async()=>{
    setAL(true);
    try{const[u,r]=await Promise.all([db.getUsers(),db.getRecords()]);setUsers(u||[]);setRecords(r||[]);}
  ...
  const handleLogin=async user=>{
    setCU(user);
    if(USE_SUPABASE) await loadRemote();
  };
  ...
  const onEntry=async()=>{
    ...
    if(USE_SUPABASE){
      try{const res=await db.insertRecord({user_id:currentUser.id,entry,exit:null});if(res&&res[0])setRecords(p=>[res[0],...p]);}catch(e){console.error(e);}
    } else {
      setRecords(p=>[{id:Date.now(),user_id:currentUser.id,userId:currentUser.id,entry,exit:null},...p]);
    }
    setAcL(false);
  };
  ...
  const onExit=async()=>{
    ...
    if(USE_SUPABASE){
      try{const res=await db.updateRecord(active.id,{exit});if(res&&res[0])setRecords(p=>p.map(r=>r.id===active.id?res[0]:r));}catch(e){console.error(e);}
    } else {
      setRecords(p=>p.map(r=>r.id===active.id?{...r,exit}:r));
    }
    setAcL(false);
  };

  const logout=()=>{setCU(null);if(USE_SUPABASE){setUsers([]);setRecords([]);}else{setUsers(LOCAL_USERS);setRecords([]);}};
  ...
  if(currentUser.role==="admin") return <AdminPanel users={users} setUsers={setUsers} records={records} setRecords={setRecords} onLogout={logout}/>;
